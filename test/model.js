(function(QUnit) {
  ...
  QUnit.test('underscore methods', function(assert) {
    assert.expect(5);
    var model = new Schmackbone.Model({foo: 'a', bar: 'b', baz: 'c'});
  ...
  QUnit.test('chain', function(assert) {
    var model = new Schmackbone.Model({a: 0, b: 1, c: 2});
    assert.deepEqual(model.chain().pick('a', 'b', 'c').values().compact().value(), [1, 2]);
  });

  QUnit.test('clone', function(assert) {
    assert.expect(10);
    var a = new Schmackbone.Model({foo: 1, bar: 2, baz: 3});
    var b = a.clone();
    ...
    var foo = new Schmackbone.Model({p: 1});
    var bar = new Schmackbone.Model({p: 2});
  ...
  QUnit.test('isNew', function(assert) {
    ...
    a = new Schmackbone.Model({foo: 1, bar: 2, baz: 3, id: -5});
    assert.ok(!a.isNew(), 'any defined ID is legal, negative or positive');
    a = new Schmackbone.Model({foo: 1, bar: 2, baz: 3, id: 0});
    assert.ok(!a.isNew(), 'any defined ID is legal, including zero');
    assert.ok(new Schmackbone.Model().isNew(), 'is true when there is no id');
    assert.ok(!new Schmackbone.Model({id: 2}).isNew(), 'is false for a positive integer');
    assert.ok(!new Schmackbone.Model({id: -5}).isNew(), 'is false for a negative integer');
  });
  ...
  QUnit.test('matches with predicate', function(assert) {
    var model = new Schmackbone.Model({a: 0});
  ...
  QUnit.test('set and unset', function(assert) {
    assert.expect(8);
    var a = new Schmackbone.Model({id: 'id', foo: 1, bar: 2, baz: 3});
    var changeCount = 0;
  ...
  QUnit.test('set falsy values in the correct order', function(assert) {
    assert.expect(2);
    var model = new Schmackbone.Model({result: 'result'});
  ...
  QUnit.test('set an empty string', function(assert) {
    assert.expect(1);
    var model = new Schmackbone.Model({name: 'Model'});
    model.set({name: ''});
  ...
  QUnit.test('clear', function(assert) {
    assert.expect(3);
    var changed;
    var model = new Schmackbone.Model({id: 1, name: 'Model'});
  ...
  QUnit.test('defaults', function(assert) {
    assert.expect(9);
    var Defaulted = Schmackbone.Model.extend({
      defaults: {
        one: 1,
        two: 2
      }
    ...
    Defaulted = Schmackbone.Model.extend({
      defaults: function() {
        return {
          one: 3,
          two: 4
  ...
  QUnit.test('change, hasChanged, changedAttributes, previous, previousAttributes', function(assert) {
    assert.expect(9);
    var model = new Schmackbone.Model({name: 'Tim', age: 10});
  ...
  QUnit.test('changedAttributes', function(assert) {
    assert.expect(3);
    var model = new Schmackbone.Model({a: 'a', b: 'b'});
    assert.deepEqual(model.changedAttributes(), false);
  ...
  QUnit.test('change with options', function(assert) {
    assert.expect(2);
    var value;
    var model = new Schmackbone.Model({name: 'Rob'});
  ...
  QUnit.test('change after initialize', function(assert) {
    ...
    var obj = new Schmackbone.Model(attrs);
  ...
  QUnit.test('save within change event', function(assert) {
    assert.expect(1);
    var env = this;
    var model = new Schmackbone.Model({firstName: 'Taylor', lastName: 'Swift'});
    model.url = '/test';
    model.on('change', function() {
      model.save();
  ...
  QUnit.test('validate after save', function(assert) {
    assert.expect(2);
    var lastError, model = new Schmackbone.Model();
  ...
  QUnit.test('save will pass extra options to success callback', function(assert) {
    assert.expect(1);
    var SpecialSyncModel = Schmackbone.Model.extend({
      sync: function(method, m, options) {
        _.extend(options, {specialSync: true});
        return Schmackbone.Model.prototype.sync.call(this, method, m, options);
      },
      urlRoot: '/test'
    });
  ...
  QUnit.test('validate on unset and clear', function(assert) {
    assert.expect(6);
    var error;
    var model = new Schmackbone.Model({name: 'One'});
    model.validate = function(attrs) {
      if (!attrs.name) {
        error = true;
        return 'No thanks.';
  ...
  QUnit.test('Inherit class properties', function(assert) {
    assert.expect(6);
    var Parent = Schmackbone.Model.extend({
  ...
  QUnit.test('hasChanged/set should use same comparison', function(assert) {
    assert.expect(2);
    var changed = 0, model = new Schmackbone.Model({a: null});
    model.on('change', function() {
  ...
  QUnit.test('set value regardless of equality/change', function(assert) {
    assert.expect(1);
    var model = new Schmackbone.Model({x: []});
    var a = [];
    model.set({x: a});
  ...
  QUnit.test('set same value does not trigger change', function(assert) {
    assert.expect(0);
    var model = new Schmackbone.Model({x: 1});
  ...
  QUnit.test('unset does not fire a change for undefined attributes', function(assert) {
    assert.expect(0);
    var model = new Schmackbone.Model({x: undefined});
  ...
  QUnit.test('`previous` for falsey keys', function(assert) {
    assert.expect(2);
    var model = new Schmackbone.Model({'0': true, '': true});
  ...
  QUnit.test('`save` with `wait` sends correct attributes', function(assert) {
    assert.expect(5);
    var changed = 0;
    var model = new Schmackbone.Model({x: 1, y: 2});
    model.url = '/test';
  ...
  QUnit.test('#1943 change calculations should use _.isEqual', function(assert) {
    var model = new Schmackbone.Model({a: {key: 'value'}});
  ...
  QUnit.test('isValid', function(assert) {
    var model = new Schmackbone.Model({valid: true});
    model.validate = function(attrs) {
  ...
  QUnit.test('mixin', function(assert) {
    Schmackbone.Model.mixin({
      isEqual: function(model1, model2) {
        return _.isEqual(model1, model2.attributes);
      }
    });

    var model1 = new Schmackbone.Model({
      a: {b: 2}, c: 3
    });
    var model2 = new Schmackbone.Model({
      a: {b: 2}, c: 3
    });
    var model3 = new Schmackbone.Model({
      a: {b: 4}, c: 3
    });
